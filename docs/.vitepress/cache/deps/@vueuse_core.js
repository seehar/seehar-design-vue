...
// node_modules/.pnpm/vue-demi@0.13.8_vue@3.2.37/node_modules/vue-demi/lib/index.mjs
var isVue2 = false
var isVue3 = true
...
function useDateFormat(date, formatStr = 'HH:mm:ss', options = {}) {
    return computed(() =>
        formatDate(
            ...
            options == null ? void 0 : options.locales
        )
    )
}
    ...
    const cleanup = [
        ...
        detectIframe &&
            useEventListener(window2, 'blur', event => {
                ...
                const el = unrefElement(target)
                if (
                    ((_a2 = document.activeElement) == null ? void 0 : _a2.tagName) === 'IFRAME' &&
    ...
    const element = customRef((track, trigger) => {
        ...
        return {
            get() {
                var _a2, _b
                track()
                return (_b = (_a2 = instance == null ? void 0 : instance.proxy) == null ? void 0 : _a2.$refs[key]) !=
                    null
                    ? _b
                    : initialValue
            },
    ...
    tasks.reduce((prev, curr) => {
        return prev
            .then(prevRes => {
                var _a2
                if (
                    ((_a2 = result[activeIndex.value]) == null ? void 0 : _a2.state) === promiseState.rejected &&
                    interrupt
                ) {
                    onFinished()
                    return
...
var DEVICE_PIXEL_RATIO_SCALES = [1, 1.325, 1.4, 1.5, 1.8, 2, 2.4, 2.5, 2.75, 3, 3.5, 4]
...
function useElementVisibility(element, { window: window2 = defaultWindow, scrollTarget } = {}) {
    ...
    if (window2) {
        tryOnMounted(() =>
            useEventListener(() => resolveUnref(scrollTarget) || window2, 'scroll', testBounding, {
                capture: false,
                passive: true
            })
        )
...
function createFetch(config = {}) {
    ...
    function useFactoryFetch(url, ...args) {
        const computedUrl = computed(() =>
            config.baseUrl ? joinPaths(resolveUnref(config.baseUrl), resolveUnref(url)) : resolveUnref(url)
        )
        ...
        if (config.payload) {
            ...
            if (config.payloadType)
                headers['Content-Type'] =
                    (_a22 = payloadMapping[config.payloadType]) != null ? _a22 : config.payloadType
...
function useFocus(target, options = {}) {
    ...
    const focused = computed({
        get() {
            return (
                isDef(activeElement.value) && isDef(targetElement.value) && activeElement.value === targetElement.value
            )
        },
        set(value) {
            var _a2, _b
...
function useTitle(newTitle = null, options = {}) {
    var _a2, _b
    const { document: document2 = defaultDocument, observe = false, titleTemplate = '%s' } = options
    ...
    watch(
        title,
        (t, o) => {
            if (isString(t) && t !== o && document2) document2.title = format(t)
        },
        { immediate: true }
    )
